...
class Discriminator(nn.Module):
    def __init__(self,DIM=16,channels=1):
        super(Discriminator, self).__init__()
        self.main = nn.Sequential(
            # input is 1 x 330 x 261
            nn.Conv2d(channels, DIM, kernel_size=3, stride=2, padding=1, bias=False),
...
class WGAN_GP(object):
    ...
    def train(self, train_loader):
        ...
        for g_iter in range(self.generator_iters):
            ...
            for d_iter in range(self.critic_iter):
                ...
                z = torch.rand((self.batch_size, 10, 1, 1))
                ...
                z = self.get_torch_variable(torch.randn(self.batch_size, 10, 1, 1))
            ...
            if (g_iter) % SAVE_PER_TIMES == 0:
                ...
                z = self.get_torch_variable(torch.randn(800, 10, 1, 1))
                samples = self.G(z)
    ...
    def real_images(self, images, number_of_images):
        if (self.C == 3):
            return self.to_np(images.view(-1, self.C, 330, 261)[:self.number_of_images])
        else:
            return self.to_np(images.view(-1, 330, 261)[:self.number_of_images])
    ...
    def generate_img(self, z, number_of_images):
        ...
        for sample in samples:
            if self.C == 3:
                generated_images.append(sample.reshape(self.C, 330, 261))
            else:
                generated_images.append(sample.reshape(330, 261))
    ...
    def save_model(self):
        torch.save(self.G.state_dict(), './0904/generator.pkl')
        torch.save(self.D.state_dict(), './0904/discriminator.pkl')
        print('Models save to ./generator.pkl & ./discriminator.pkl ')

    def save_model_iter(self,g_iter):
        torch.save(self.G.state_dict(), './params/0904/generator_{}.pkl'.format(str(g_iter).zfill(3)))
        torch.save(self.D.state_dict(), './params/0904/discriminator_{}.pkl'.format(str(g_iter).zfill(3)))
        print('Models save to ./generator.pkl & ./discriminator.pkl ')
    ...
    def generate_latent_walk(self, number):
        ...
        for i in range(1, number_int + 1):
            ...
            images.append(fake_im.view(self.C,330,261).data.cpu())
